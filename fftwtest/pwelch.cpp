...
std::pair<std::vector<double>, std::vector<double>> welchPSD(py::array_t<double, py::array::c_style | py::array::forcecast> inputArray, int nperseg, int noverlap, double fs, const std::string &scaling)
{
    ...
    std::vector<double> psd(nperseg / 2 + 1);
    std::vector<double> freqs(nperseg / 2 + 1);
        ...
        freqs[i] = i * fs / nperseg;
    {
        ...
        for (int i = 0; i < num_segments; i++)
        {
            ...
            for (int j = 0; j < nperseg / 2 + 1; j++)
                psd[j] += (out[j][0] * out[j][0] + out[j][1] * out[j][1])* scale / num_segments;
